...
<div id="plugin_acl">
	...
	if(!isset($no_acl_links))
	{
        ...
        $links = array();
        $links[] = $this->Html->link(__d('acl', 'Permissões'), '/admin/acl/aros/index', array('class' => ($selected == 'aros' )? 'selected' : null));
        $links[] = $this->Html->link(__d('acl', 'Ações'), '/admin/acl/acos/index', array('class' => ($selected == 'acos' )? 'selected' : null));
