...
<div>
	...
	<?php
	echo $this->Html->link($this->Html->image('/acl/img/design/cross.png') . ' ' . __d('acl', 'Limpar tabela de permissões'), '/admin/acl/aros/empty_permissions', array('confirm' => __d('acl', 'Tem certeza de que deseja excluir todos os grupos e permissões de usuários?'), 'escape' => false));
	?>
...
<tr>
	<th></th>
	<th><?php echo __d('acl', 'Dar acesso para <em>todas as ações</em>'); ?></th>
	<th><?php echo __d('acl', 'Bloquear todas as ações para <em>todas as ações</em>'); ?></th>
</tr>
		    	...
		    	foreach($roles as $role)
		    	{
    		    	...
    		    	*/
    		        ...
    		        if(!in_array($controller_name . '_' . $role[$role_model_name][$role_pk_name], $js_init_done))
    		        {
    		        	...
    		        	$this->Js->buffer('init_register_role_controller_toggle_right("' . $this->Html->url('/', true) . '", "' . $role[$role_model_name][$role_pk_name] . '", "", "' . $controller_name . '", "' . __d('acl', 'Uma ações está faltando. Reconstrua a lista de ações.') . '");');
    		        }
        	        ...
        	        foreach($roles as $role)
    		    	{
    		    	    ...
    		    	    */
	    		    	...
	    		    	if(!in_array($plugin_name . "_" . $plugin_ctrler_name . '_' . $role[$role_model_name][$role_pk_name], $js_init_done))
	    		        {
	    		        	...
	    		        	$this->Js->buffer('init_register_role_controller_toggle_right("' . $this->Html->url('/', true) . '", "' . $role[$role_model_name][$role_pk_name] . '", "' . $plugin_name . '", "' . $plugin_ctrler_name . '", "' . __d('acl', 'Uma ações está faltando. Reconstrua a lista de ações.') . '");');
	    		        }
