    ...
    <?php echo $this->Html->scriptStart(array('block'=>'codesJavaScripts')); ?>
      $(document).ready(function(){
         ...
         $("#CandidatoCondicaoComprimentoCota").click(function(){
              if($("#CandidatoCondicaoComprimentoCota").is(":checked")){
                $("#comprimento_cota_caracteristica").fadeIn();
                $("#CandidatoComprimentoCotaCaracteristicas").attr('required', 'true');
              }else{
                $("#comprimento_cota_caracteristica").fadeOut();
                $("#CandidatoComprimentoCotaCaracteristicas").attr('required', 'false');
              }
         });
         ...
         <?php if($this->request->data['Candidato']['tem_filhos'] == 'S'): ?>
            $("#temFilhos").fadeIn();
            $("#CandidatoQuantosFilhos").attr('required', 'true');
         <?php endif; ?>

         <?php if($this->request->data['Candidato']['possui_deficiencia_auditiva'] == 1): ?>
            $("#auditiva").fadeIn();
            $("#CandidatoDescricaoDeficienciaAuditiva").attr('required', 'true');
         <?php endif; ?>

         <?php if($this->request->data['Candidato']['possui_deficiencia_fisica'] == 1): ?>
            $("#fisica").fadeIn();
            $("#CandidatoDescricaoDeficienciaFisica").attr('required', 'true');
         <?php endif; ?>

         <?php if($this->request->data['Candidato']['possui_deficiencia_intelectual'] == 1): ?>
            $("#intelectual").fadeIn();
            $("#CandidatoDescricaoDeficienciaIntelectual").attr('required', 'true');
         <?php endif; ?>

         <?php if($this->request->data['Candidato']['possui_deficiencia_visual'] == 1): ?>
            $("#visual").fadeIn();
            $("#CandidatoDescricaoDeficienciaVisual").attr('required', 'true');
         <?php endif; ?>

         <?php if($this->request->data['Candidato']['necessita_ajudas_tecnicas'] == 1): ?>
            $("#tecnicas").fadeIn();
            $("#CandidatoQuaisAjudasTecnicas").attr('required', 'true');
         <?php endif; ?>

         <?php if($this->request->data['Candidato']['condicao_comprimento_cota'] == 1): ?>
            $("#reabilitacao").fadeIn();
            $("#CandidatoCondicaoComprimentoCota1").attr('checked', 'true');
            $("#CandidatoDescricaoReabilitacao").attr('required', 'true');
         <?php endif; ?>

         <?php if($this->request->data['Candidato']['condicao_comprimento_cota'] == 0): ?>
            $("#CandidatoCondicaoComprimentoCota0").attr('checked', 'true');
         <?php endif; ?>
          ...
          $('#CandidatoCursos').tokenfield({
            ...
            <?php if (!empty($this->request->data['CursosAcademicosTecnico'])):?>
            tokens:[
                <?php foreach($this->request->data['CursosAcademicosTecnico'] as $curso): ?>
                    {value: '<?=$curso['id']?>', label: '<?=$tiposFormacoes[$curso['tipo']].' - '.$curso['nome']?>'},
                <?php endforeach;?>
            ],
            <?php endif; ?>
          });

      	  var $validator = $("#CandidatoEditForm").validate({
          ...
          $('#rootwizard').bootstrapWizard({
         		...
         		'onNext': function(tab, navigation, index) {
         			var $valid = $("#CandidatoEditForm").valid();
         			if(!$valid) {
                       ...
                       <div class="tab-pane fade" id = "tabs-5">
                        	<div class="row">
                                <?php
                                    $numExperiencias = 1;
                                    $i = 1;
                                    if(!empty($this->data['ExperienciasProfissional'])):
                                        $numExperiencias = count($this->data['ExperienciasProfissional']);
                                        foreach($this->data['ExperienciasProfissional'] as $key=>$experiencia):
                                ?>
                                            <div class="col-lg-4">
                                                <div class="panel panel-default">
                                                    <div class="panel-heading">Experiência <?=$i?></div>
                                                    <!-- /.panel-heading -->
                                                    <div class="panel-body">
                                                        <?php
                                                  		    echo $this->Form->input('id', array('type'=>'hidden', 'name'=>'data[ExperienciasProfissional]['.$key.'][id]', 'value'=>$experiencia['id'], 'class' => 'form-control'));
                                                  		    echo $this->Form->input('empresa', array('name'=>'data[ExperienciasProfissional]['.$key.'][empresa]', 'value'=>$experiencia['empresa'], 'class' => 'form-control'));
                                                  		    echo $this->Form->input('admissao', array('name'=>'data[ExperienciasProfissional]['.$key.'][admissao]', 'value'=>$this->Util->format_date($experiencia['admissao']), 'label'=>'Admissão', 'onkeyup'=>'formataData(this,event)', 'onkeypress'=>'return OnlyNumbers(event)', 'type'=>'text', 'maxlength'=>10, 'class' => 'form-control'));
                                                  		    echo $this->Form->input('demissao', array('name'=>'data[ExperienciasProfissional]['.$key.'][demissao]', 'value'=>$this->Util->format_date($experiencia['demissao']), 'label'=>'Demissão', 'onkeyup'=>'formataData(this,event)', 'onkeypress'=>'return OnlyNumbers(event)', 'type'=>'text', 'maxlength'=>10, 'class' => 'form-control'));
                                                  		    echo $this->Form->input('cargo', array('name'=>'data[ExperienciasProfissional]['.$key.'][cargo]', 'value'=>$experiencia['cargo'], 'class' => 'form-control'));
                                                  		    echo $this->Form->input('ultimo_salario', array('name'=>'data[ExperienciasProfissional]['.$key.'][ultimo_salario]', 'value'=>$experiencia['ultimo_salario'], 'label'=>'Último Salário', 'onkeypress'=>'return OnlyNumbers(event)', 'onkeyup' => 'formataValor(this,event)', 'class' => 'form-control'));
                                                            echo $this->Form->input('atividades_desenvolvidas', array('name'=>'data[ExperienciasProfissional]['.$key.'][atividades_desenvolvidas]', 'value'=>$experiencia['atividades_desenvolvidas'], 'class' => 'form-control'));
                                                  		    echo $this->Form->input('motivo_desligamento', array('name'=>'data[ExperienciasProfissional]['.$key.'][motivo_desligamento]', 'value'=>$experiencia['motivo_desligamento'], 'class' => 'form-control'));
                                                        ?>
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                ?>
                                            <div class="col-lg-4">
                                                <div class="panel panel-default">
                                                    ...
                                                    <div class="panel-body">
                                                        <?php
                                                		    echo $this->Form->input('candidato_id', array('type'=>'hidden', 'name'=>'data[ExperienciasProfissional]['.$y.'][candidato_id]', 'value'=>$this->data['Candidato']['id'], 'class' => 'form-control'));
                                    ...
                                    <div class="panel panel-default">
                                        ...
                                        <div class="panel-body">
                                            <?php
                                        		...
                                        		echo $this->Form->input('descricao_deficiencia_auditiva', array('div'=>array('id'=>'auditiva', 'style'=>'display: none;'), 'label'=>'Descrição da Deficiência Auditiva', 'class' => 'form-control'));
                                        		...
                                        		echo $this->Form->input('descricao_deficiencia_fisica', array('div'=>array('id'=>'fisica', 'style'=>'display: none;'), 'label'=>'Descrição da Deficiência Física', 'class' => 'form-control'));
                                        		...
                                        		echo $this->Form->input('descricao_deficiencia_intelectual', array('div'=>array('id'=>'intelectual', 'style'=>'display: none;'), 'label'=>'Descrição da Deficiência Intelectual', 'class' => 'form-control'));
                                        		...
                                        		echo $this->Form->input('descricao_deficiencia_visual', array('div'=>array('id'=>'visual', 'style'=>'display: none;'), 'label'=>'Descrição da Deficiência Visual', 'class' => 'form-control'));
                                        		...
                                        		echo $this->Form->input('quais_ajudas_tecnicas', array('div'=>array('id'=>'tecnicas', 'style'=>'display: none;'), 'label'=>'Quais Ajudas Técnicas', 'class' => 'form-control'));
                                        	?>
                                        </div>
                                    </div>
                                </div>
