...
{
	echo '<h3>' . __d('acl', 'Grupos não correspondem ao Aro') . '</h3>';
...
{
	echo '<h3>' . __d('acl', 'Usuários não correspondem ao Aro') . '</h3>';
...
{
	echo '<p>';
	echo $this->Html->link(__d('acl', 'Carregar'), '/admin/acl/aros/check/run');
	echo '</p>';
}
else
{
	echo '<p>';
	echo __d('acl', 'Não há usuário faltando.');
	echo '</p>';
}
?>
