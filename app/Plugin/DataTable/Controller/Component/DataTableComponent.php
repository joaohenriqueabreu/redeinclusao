...
class DataTableComponent extends Component {
    ...
    public function paginate($name = null, $scope = array()) {
        ...
        if (isset($this->settings[$name]['model'])) {
            if ($this->settings[$name]['model'] != $name) {
                $novoNome = $this->settings[$name]['model'];
            } else {
                $novoNome = $name;
            }
        } else {
        ...
        // Alteração para seta o nome da tabela antes do nome do campo evitando ambiguidades no SQL
        $keys = array_keys($this->settings[$name]['conditions']);
        $totalKeys = count($keys);

        $conditions = $this->settings;
        $conditions[$novoNome]['conditions'] = [];

        for ($i = 0; $i < $totalKeys; $i ++) {
            if (!strrchr($keys[$i], '.') && !is_int($keys[$i])) {
                $conditions[$novoNome]['conditions'][$novoNome . '.' . $keys[$i]] = $this->settings[$name]['conditions'][$keys[$i]];
            }
        }
        // Fim da alteração

        $config = new DataTableConfig($name, $conditions);
...
//        exit();
    ...
    protected function _sort(DataTableConfig $config) {
        ...
        for ($i = 0; $i < count($config->columns); $i ++) {
            ...
            $config->order[$column] = in_array(strtolower($direction), array(
                        'asc',
                        'desc'
                    )) ? $direction : 'asc';
        }
